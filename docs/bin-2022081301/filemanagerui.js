...
/** maximu filesize for a single file in the zip archive. 
 * @type {number} */
const FileManagerUI_UNCOMPRESS_FILE_SIZE_LIMIT = 50 * 1000 * 1000;

// JSZip : https://github.com/Stuk/jszip
// <script src="https://cdnjs.cloudflare.com/ajax/libs/jszip/3.10.1/jszip.min.js" integrity="sha512-XMVd28F1oH/O71fzwBnV7HucLxVwtxf26XV8P4wPk26EDxuGZ91N8bsOttmnomcCD3CS5ZMRL50H0GgOHvegtg==" crossorigin="anonymous" referrerpolicy="no-referrer"></script>


function prepareDataDirectory(path) {
    try {
        FS.mkdir(path);
    } catch {
        // NOP
...
class FileManagerUI {
    ...
    constructor(mode) {
        ...
        /** @type {string[]}  */
        this.__extension_filters = [".slvs", ".zip"];
     ...
     * @param {File} zipfile 
     */
    async __extractZip(zipfile) {
        return new Promise((resolve, reject) => {
            const jszip = new JSZip();
            const filereader = new FileReader();
            const readFileAsync = new Promise((filereader_resolve, filereader_reject) => {
                filereader.onerror = (err) => filereader_reject(err);
                filereader.onload = (result) => filereader_resolve(result.target.result);
                filereader.readAsArrayBuffer(zipfile);
            });

            readFileAsync.then((buffer) => {
                jszip.loadAsync(buffer)
                    .then(async (zipobject) => {
                        console.log("loadAsync() OK:", zipobject);
                        console.log(`Folders:`, zipobject.folder(/.*/));
                        console.log(`Files:`, zipobject.file(/.*/));
                        const fileentries = zipobject.file(/.*/);

                        for (let i = 0; i < fileentries.length; i++) {
                            const fileentry = fileentries[i];
                            const filename = fileentry.name.replaceAll(/\//g, '_');
                            const inflatedSize = fileentry._data.uncompressedSize;
                            //FIXME(emscripten):
                            if (inflatedSize > FileManagerUI_UNCOMPRESS_FILE_SIZE_LIMIT) {
                                throw new Error(`File in zip looks too large after inflated: "${filename}" will be ${inflatedSize} bytes.`);
                            }
                            const blob = await fileentry.async("blob");
                            const opts = {
                                lastModified: zipobject.date
                            };
                            if (filename.toUpperCase().endsWith(".SLVS")) {
                                opts.type = "application/x-solvespace";
                            }
                            const fileobject = new File([blob], filename, opts);
                            await this.__tryAddFile(fileobject);
                        }

                        resolve();
                    })
                    .catch((err) => {
                        console.error(`error in JSZip.loadAync():`, err);
                        reject(err);
                    })
            })
            .catch((err) => {
                console.error(`error in readFileAsync():`, err);
                reject(err);
            })    
        });
    }
    ...
    async __tryAddFile(file) {
        return new Promise(async (resolve, reject) => {
            if (!file) {
                ...
                alert("Specified file is larger than 10MB. Canceced.");
                reject(new Error(`File is too large: "${file.name} is ${file.size} bytes`));
                
            } else {
                if (file.name.toUpperCase().endsWith(".ZIP") || file.type == "application/zip") {
                    this.__extractZip(file)
                        .then(() => {
                            resolve();
                        });
